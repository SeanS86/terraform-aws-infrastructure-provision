...
# Kong Configuration for Kubernetes Dashboard NodePort Access
...
The `curl -vki https://<worker-node-ip>:30865` test successfully returning the dashboard page (after an initial `400 Bad Request` when using HTTP) indicates that Kong is indeed listening for HTTPS traffic on the port mapped by NodePort `30865` and is correctly proxying it to the Kubernetes Dashboard.
